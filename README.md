## Introduction
This adds to the existing OpenShift Docker Builder the possibility to ad-hoc inject HTTP_PROXY environment variables.
Can also be used to "just-in-time" inject anything else.

## Create custom ose-docker-builder
```
oc new-project openshift3
oc import-image ose-docker-builder-upstream --from=registry.access.redhat.com/openshift3/ose-docker-builder:latest --confirm
oc new-build --image-stream=ose-docker-builder-upstream --code=https://github.com/sterburg/ose-docker-builder.git --strategy=docker
```

## Preparing nodes to pull the new ose-docker-builder
```
...
ansible  -m shell -a "docker pull docker-registry-default.openshift.company.nl/openshift3/ose-docker-builder:latest" nodes
ansible  -m shell -a "docker tag  docker-registry-default.openshift.company.nl/openshift3/ose-docker-builder:latest openshift3/ose-docker-builder:latest" nodes
```
